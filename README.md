...
- **gulp-rename**: allows you to rename files, such as adding .min to a file extension when a file is minified
...
- **gulp-plumber**: helps make our gulp processes more resilient by preventing errors from breaking our processes
- **browser-sync**: adds a local livereload server
...
We need a project folder to hold all of our files.  This project directory needs subdirectories for both our source files and the files we'll have Gulp.js generate for us.  It will look like this when you've completed these steps:
...
1. When a new Terminal window opens, you should be in your user home directory by default.  You can confirm this by typing `pwd`, which is the command used in Terminal to display the current working directory.  If you're not in your /Users/*yourusername* directory, just type `cd ~`.  The **cd** is the command to *change directory* and the tilde tells it to change to your home directory.
1. We're going to put the project folder, which I'll name startersite, on the desktop, so type: ` mkdir -p ~/Desktop/startersite`. The **mkdir** command means *make directory* and the part that follows is the path and name of the folder we want to create.  Remember, the ~ means start in your home directory.  (We're already in the home directory, so we could have skipped the tilde and forward slash, but we're practicing so that we get used to using it. :smile:). When you see one or more letters preceded by a hyphen used in a Terminal command, we call that a flag.  The p flag (`-p`) used with the mkdir command means create any folder(s) in the path that don't exist. If you had typed `mkdir -p ~/Desktop/mysites/portfolio` for example, and there wasn't a mysites folder already on the desktop, it would have created both the mysites folder and the nested portfolio folder.
1. Next, type `cd !$`.  The **!$** is another one of those special commands we can give to change directory.  It will take us into the folder we just created.<br><br>
1. From here we need to build our project folders.  We could do these one at a time by using our `mkdir` command, but we can make several folders which are all on the same level with one command by wrapping them in curly braces like this: `mkdir -p assets/{css,img,js} src/{js,sass}`. Notice that the **mkdir** command allows us to make multiple folders with a single command by placing a space between the folder names.  When using the curly braces to wrap multiple nested folders, the nested folder names are separated by a comma.<br><br>
...
1. Next, we're going to create an index.html file in the root of our project.  Once you're back in the project folder, type `touch index.html`.  I know -- it's a creepy sounding command, but it just means *if this file doesn't exist, create it*.
1. We're going to use touch once more now to create our sass file, so type `touch src/sass/styles.scss`.
...
> ### Use tree to show files in Terminal
> The ls command can be a lot of work to check out your folder structure so we can use another command borrowed from Linux to show the entire tree view of our directory right in the Terminal.  Unfortunately, the tree command doesn't come native with MAC OS, but it's easy enough to install with Homebrew using `brew install tree`.  If you don't have Homebrew, you can use `sudo port install tree`.  That **sudo** command means run this operation as the owner of this machine. It actually stands for *super user do* so it will prompt you for your password (the one you use to login to your computer) just to be certain that you are authorized to install software on it.<br><br>
> Once tree is installed, you can type `tree -d -L 2`. The **tree** command is being used here with some flags to limit our results.  The d flag tells it to only display folders and ignore any files.  The L flag is for limiting the number of levels to display.  In this case we're only showing 2 levels deep.  Another helpful flag is the `--filelimit` flag followed by the number of folders we want to show in any given directory.<br><br>
> By the way, in case you're curious why some flags have a single hyphen and other have two, like this filelimit flag, it has to do with the fact that single character flags can be written in a shorthand format joined together following a single hyphen (e.g., `tree -dtL 2` is the same as `tree -d -t -L 2`). As a result, flags that have more than one character in their name have to be preceded with two hyphens or the computer would interpret them using the shorthand rules (i.e., `-filelimit` would be interpreted as `-f -i -l -e -l -i -m -i -t`).

### Initialize the Project for Git

1. We're going to want to use git for our project, so to initialize the folder, we just need to make sure we're in our project folder by typing `pwd`.  If you need to, cd into the correct folder with `cd ~/Desktop/startersite`.  Once there, type `git init`.  You should get back a message that reads: **Initialized empty Git repository in /Users/yourusername/Desktop/test/.git/**.
...
1. We're going to add some stuff to this file by opening it in our Atom text editor by typing: `atom .gitignore`.
...
1. To install Gulp.js locally for this project, we need to first install the Gulp CLI (command line interface).  We'll install this CLI globally on our machine so that we only need to do it once and can reuse it on any of our projects in the future.  The command to do this is `npm install gulp-cli -g`. The -g flag says install this feature globally on my machine.  It means you can then run its commands in the Terminal from any directory. 
1. After the cli is installed, we need to install Gulp.js into our project, so first make sure you're in the project folder with `cd ~/Desktop/startersite`, then type `npm install gulp --save-dev`.  The npm install command is just like the one we just used except this one is only installing a local copy for the project so we don't need sudo.  The --save-dev option tells npm to update our package.json file by adding information about Gulp.js to the devDependencies.  This means that we only need Gulp.js when we're developing our website or app, but not later when we are running this project in production.
